...
- 페이지: `/` 종목 분석 · `/allocate` 자산 분배(총자산·비율 → 배분금액·매수 주수, 소수점 6자리 매수 기준). 하단 탭바(`src/components/NavBar.tsx`, layout에 상주)로 이동. 분배 계산은 원화 정본 + 표시 통화(₩/$) 토글 — 미국 종목은 `EnrichedStock.priceKrw`(= price × usdKrw)를 원화 단가로 쓰므로 환율 코드가 따로 없음.
